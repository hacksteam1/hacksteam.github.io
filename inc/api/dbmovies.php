...
<div id="dt_importer" class="dt_importer_wrap">
	...
	<div class="dt_importer_contaiter">
		<div class="dt_imp_menu">
			<ul class="tabs">
				<li id="filter_year_li" class="tab-link current" data-tab="tab-1"><?php _d('Filtrar por ano'); ?></li>
				<li id="filter_year_li" class="tab-link" data-tab="tab-0"><?php _d('Título da pesquisa'); ?></li>
				<li id="single_url_li" class="tab-link" data-tab="tab-2"><?php _d('ID de ImportaçãoD'); ?></li>
		...
		<div class="content">
			<div id="tab-0" class="tab-content">
				<h1 style="margin-top: 0;"><?php _d('Pesquisar conteúdo'); ?></h1>
				<form id="search_all" class="search_all">
					<div class="box">
						<input type="text" name="query" placeholder="<?php _d('Pesquisar um título ..'); ?>">
						<button name="search_all_data" type="submit" class="button button-primary"><?php _d('Procurar'); ?></button>
					</div>
					<label for="page"><input type="number" name="page"></label>
					<label for="movie"><input type="radio" id="movie" name="type" value="movie" required checked> <?php _d('Filmes'); ?></label>
					<label for="tvshows"><input type="radio" id="tvshows" name="type" value="tv" required> <?php _d('Séries'); ?></label>
					<?php wp_nonce_field('search-all','search-all-nonce') ?>
				</form>
			</div>
			<div id="tab-1" class="tab-content current">
				<section style="padding-left:0">
					<h1 style="margin-top: 0;"><?php _d('Filmes'); ?></h1>
					<form id="search_imdb" class="form_importer_dt">
						<p>
						<input type="number" id="imdbyear" name="imdbyear" placeholder="<?php _d('Ano'); ?>" min="1930" max="<?php echo date('Y'); ?>" required>
						<input style="margin-right: 0" type="number" id="imdbpage" name="imdbpage" placeholder="<?php _d('Página'); ?>" min="1" required>
						</p>
						<p><input type="submit" class="button button-primary" name="search_data_imdb" value="<?php _d('Obter conteúdo'); ?>"></p>
						<?php wp_nonce_field('send-imdb','send-imdb-nonce') ?>
					</form>
				</section>
				<section class="right" style="padding-right:0">
					<h1 style="margin-top: 0;"><?php _d('Séries'); ?></h1>
					<form id="search_tmdb" class="form_importer_dt">
						<p>
						<input type="number" id="tmdbyear" name="tmdbyear" placeholder="<?php _d('Ano'); ?>" min="1930" max="<?php echo date('Y'); ?>" required>
						<input style="margin-right: 0" type="number" id="tmdbpage" name="tmdbpage" placeholder="<?php _d('Página'); ?>" min="1" required>
						</p>
						<p><input type="submit" class="button button-primary" name="search_data_tmdb" value="<?php _d('Obter conteúdo'); ?>"></p>
						<?php wp_nonce_field('send-tmdb','send-tmdb-nonce') ?>
					</form>
				</section>
				<p class="desc"><?php _d('Obter dados de Themoviedb.org'); ?></p>
			</div>
			<div id="tab-2" class="tab-content">
				<section>
					<h1 style="margin-top: 0;"><?php _d('Filmes'); ?></h1>
					<form id="single_url_imdb" class="form_importer_dt">
						<p><input type="text" name="idmovie" placeholder="<?php _d('ID Filme'); ?>" required></p>
						<p><input type="submit" class="button button-primary" name="send_id_movie" value="<?php _d('Importar'); ?>"></p>
						<?php wp_nonce_field('send-movies','send-movies-nonce') ?>
						<p class="desc"><?php _d('Exemplo'); ?>: themoviedb.org/movie/<strong>14564</strong></p>
					</form>
				</section>
				...
				<section class="right">
					...
					<form id="single_url_tmdb" class="form_importer_dt">
						<p><input type="text" name="idtv" placeholder="<?php _d('ID Séries'); ?>" required></p>
						<p><input type="submit" class="button button-primary" name="send_id_tv" value="<?php _d('Importar'); ?>"></p>
						<?php wp_nonce_field('send-series','send-series-nonce') ?>
						<p class="desc"><?php _d('Examplo'); ?>: themoviedb.org/tv/<strong>1402</strong></p>
					</form>
				</section>
			</div>
			<div id="tab-3" class="tab-content">
				<h1 style="margin-top: 0;"><?php _d('Status dos processos do servidor'); ?></h1>
				<div id="result_server"></div>
				<form id="api_status" class="form_importer_dt">
					<?php wp_nonce_field('send-status','send-status-nonce') ?>
					<p><input type="submit" class="button button-primary" value="<?php _d('Verificar status do servidor'); ?>"></p>
				</form>
			</div>
...
function dbmovies_assets() {
	if($_GET['page']=='dbmovies.org') {
		...
		wp_localize_script('dt-importer-tool-scripts', 'DTapi', array(
			...
			// Mensajes
			'preresultado'		=> __d('Procurando conteúdo, espere um momento...'),
			'preresultadolog'	=> __d('Pesquisando e extraindo dados...'),
			'resultadolog'		=> __d('Dados encontrados, processo concluído!'),
			'resultadoerror'	=> __d('Erro, sem dados...'),
			'agregandodatos'	=> __d('Adicionando dados...'),
			'agregandodatoslog' => __d('Adicionando conteúdo ao banco de dados...'),
			'procesocompleto'	=> __d('Processo concluído!'),
			'postdataerror'		=> __d('O conteúdo não pôde ser adicionado!'),
			'queryserver'		=> __d('Consulta de status do servidor...'),
			'verificationsr'	=> __d('Verificação concluída!'),
			'loading'			=> __d('Carregando...'),
			'getcontent'		=> __d('Obter conteúdo'),
		) );
	}
}
...
function dbm_search_all() {
	if ( current_user_can('manage_options')) {
		if( isset($_POST['search-all-nonce'] ) and wp_verify_nonce($_POST['search-all-nonce'], 'search-all') ) { 
			...
			echo '<h1>'. __d('Página').' <b>'.$pagex .'</b> '. __d('do').' <b>'.$total_pages.'</b> '.__d('existem').' <b>'.$total_results.'</b> '.__d('Títulos encontrados').'</h1>';
			foreach($results as $ci) {
				...
				if ($verificar) {
					echo $num. ' <span class="imported">'.__d('Importado').'</span> <strong><a href="http://themoviedb.org/'.$type.'/'. $id.'/" target="_blank">'.$title.'</a></strong> <i>('. $year .')</i>';
				} else {
					echo $num. ' <span><a class="'.$class.'" data-id="'. $id.'">'. __d('Importar') .'</a></span> <strong><a href="http://themoviedb.org/'.$type.'/'. $id.'" target="_blank">'.$title.'</a></strong> <i>('.$year.')</i>';
				}
				echo '</article>';
				$num++;
			...
			echo '</div>';
			echo '<a id="load_more_search">'.__d('Próxima página').'</a>';
...
function dbm_get_movies() {
	if ( current_user_can('manage_options')) {
		if( isset($_POST['send-imdb-nonce'] ) and wp_verify_nonce($_POST['send-imdb-nonce'], 'send-imdb') ) { 
			...
			$limit = count($results);
			$num = ($limit * $page - $limit + 1  );
			echo '<h1>'. __d('Página').' <b>'.$page .'</b> '. __d('do').' <b>'.$total_pages.'</b> '.__d('existem').' <b>'.$total_results.'</b> '.__d('Títulos encontrados').'</h1>';
			echo '<div class="dtlist">';
			foreach($results as $ci) {
				$id		= $ct[] = $ci['id'];
				$title	= $ct[] = $ci['title'];
				...
				if ($verificar) {
					echo $num. ' <span class="imported">'.__d('importado por').'</span> <strong><a href="http://themoviedb.org/movie/'. $id.'/" target="_blank">'.$title.'</a></strong> <i>('. $year .')</i>';
				} else {
					echo $num. ' <span><a class="a_import_imdb" data-id="'. $id.'">'. __d('Importar') .'</a></span> <strong><a href="http://themoviedb.org/movie/'. $id.'" target="_blank">'.$title.'</a></strong> <i>('.$year.')</i>';
				}
				echo '</article>';
				$num++;
			}
			echo '</div>';
			echo '<a id="load_more_imdb_link">'.__d('Próxima página').'</a>';
		}
	}
	die();
}
		...
		if ( current_user_can('manage_options')) {
			if( isset($_POST['send-tmdb-nonce'] ) and wp_verify_nonce($_POST['send-tmdb-nonce'], 'send-tmdb') ) { 
				...
				foreach($results as $ci) {
					...
					if ($verificar) {
						echo $num. ' <span class="imported">'.__d('Importado').'</span> <strong>'. $name .'</strong> <i>('. $year .')</i>';
					} else {
						echo $num. ' <span><a class="a_import_tmdb" data-id="'.$id.'">'.__d('Importar').'</a></span> <strong><a href="http://themoviedb.org/tv/'.$id.'/" target="_blank">'.$name.'</a></strong> <i>('.$year.')</i>';
					}
					echo '</article>';
					$num++;
				}
				echo '</div>';
				echo '<a id="load_more_tmdb_link">'.__d('Próxima página').'</a>';
			}
		}
	die();
}
	...
	if( isset($_POST['send-movies-nonce'] ) and wp_verify_nonce($_POST['send-movies-nonce'], 'send-movies') ) { 
		if (current_user_can('manage_options')) {
			...
			if (($idmovie != NULL)) {
				...
				$verificar = $wpdb->get_results( $consulta, OBJECT );
				if ($verificar) {
					echo '<strong>ERROR:</strong> '. __d('O conteúdo já existe');
				} else {
					$post_id = wp_insert_post($my_post);
					echo '<span class="import_completed">'. __d('Importado').'</span> <a href="'. esc_url( home_url() ) .'?p='. $post_id .'" target="_blank"><strong>'. $b3 .'</strong></a> ('. $a3 .')';
				}	
		...
		if (current_user_can('manage_options')) {
			...
			if (($ids != NULL)) {
				...
				if ($verificar) {
					...
					$post_id = wp_insert_post($my_post);
					echo '<span class="import_completed">'. __d('Importado').'</span> <a href="'. esc_url( home_url() ) .'?p='. $post_id .'" target="_blank"><strong>'. $name .'</strong></a> ('. $year .')';
				}
