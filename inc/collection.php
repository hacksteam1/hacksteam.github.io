...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$class = '';
		$title = 'Veja mais tarde';
		$icon = $icon_empty;
	}
	$output = '<div class="btn opt-button seeLaterButton"><a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&nonce=' . $nonce . '&post_id=' . $post_id . '&disabled=true&is_comment=' . $is_comment ) . '" class="animation-1 sl-button' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon .  '</a>' . $loader . '</div>';
	return $output;
}
...
function get_liked_icon() {
	$icon = '<i class="icon fa fa-bookmark"></i> <div class="txt">por ver</div>';
	return $icon;
} 

function get_unliked_icon() {
	$icon = '<i class="icon fa fa-bookmark-o"></i> <div class="txt">por ver</div>';
	return $icon;
} 
