...
<table class="options-table-responsive dt-options-table">
	...
	<tr id="ids_box">
		<td class="label">
			<label for="ids"><?php _d('Gerar dados'); ?></label>
			<p class="description"><?php _d('Gerar dados de<strong>imdb.com</strong>'); ?></p>
		</td>
		<td style="background: #f7f7f7" class="field">
			...
			<input type="button" class="button button-primary" name="Checkbx" value="<?php if(dt_get_meta('ids')){ _d('Atualizar dados'); } else { _d('Gerar'); } ?>">
			<p class="description"><?php _d('E.g. http://www.imdb.com/title/<strong>tt2911666</strong>/'); ?></p>
			<p id="verificador" style="display:none"><a class="button button-secundary" id="comprovate"><?php _d('Verificar conteúdo duplicado'); ?></a><p>
		</td>
	</tr>
	<?php } ?>
	<tr>
		<td colspan="2"><h3><?php _d('Imagens e trailer'); ?></h3></td>
	</tr>
	<tr id="dt_poster_box">
		<td class="label">
			<label for="dt_poster"><?php _d('Poster'); ?></label>
			<p class="description"><?php _d('Adicionar url imagem'); ?></p>
		</td>
		<td class="field">
	...
	<?php global $post_type; if ($post_type == 'movies') {?>
	<tr id="dt_backdrop_box">
		<td class="label">
			<label for="dt_backdrop"><?php _d('Pano de fundo principal'); ?></label>
			<p class="description"><?php _d('Adicionar url imagem'); ?></p>
		</td>
		<td class="field">
	...
	<tr id="imagenes_box">
		<td class="label">
			...
			<p class="description"><?php _d('Coloque aurl da imagem abaixo'); ?></p>
		</td>
		<td class="field">
			<input class="regular-text" type="text" name="imagenes" id="imagenes" value="<?php echo dt_get_meta('imagenes'); ?>">
			<input class="up_images_images button-secondary" type="button" value="<?php _d('Upload'); ?>" />
		</td>
	</tr>
	<tr id="youtube_id_box">
		<td class="label">
			<label for="youtube_id"><?php _d('Vídeo trailer'); ?></label>
			<p class="description"><?php _d('Adicionar id do Youtube'); ?></p>
		</td>
		<td class="field">
	...
	<tr>
		<td colspan="2"><h3><?php _d('IMDb.com dados'); ?></h3></td>
	</tr>
	<tr id="rating_imdb_box">
		<td class="label">
			<label for="imdbRating"><?php _d('Avaliação IMDb'); ?></label>
			<p class="description"><?php _d('Média / Votos'); ?></p>
		</td>
		<td class="field">
	...
	<tr id="Rated_box">
		<td class="label">
			<label for="Rated"><?php _d('Avaliado'); ?></label>
		</td>
		<td class="field">
	...
	<tr id="Country_box">
		<td class="label">
			<label for="Country"><?php _d('País'); ?></label>
		</td>
		<td class="field">
	...
	<?php } ?>
	<tr>
		<td colspan="2"><h3><?php _d('Themoviedb.org dados'); ?></h3></td>
	</tr>
	...
	<tr id="original_title_box">
		<td class="label">
			<label for="original_title"><?php _d('Título original'); ?></label>
		</td>
		<td class="field">
	...
	<tr id="release_date_box">
		<td class="label">
			<label for="release_date"><?php _d('Data de Lançamento'); ?></label>
		</td>
		<td class="field">
	...
	<tr id="rating_tmdb_box">
		<td class="label">
			<label for="vote_average"><?php _d('Avaliação TMDb'); ?></label>
			<p class="description"><?php _d('Média / Votos'); ?></p>
		</td>
		<td class="field">
	...
	<tr id="runtime_box">
		<td class="label">
			<label for="runtime"><?php _d('Tempo de execução'); ?></label>
		</td>
		<td class="field">
	...
	<tr id="dt_cast_box">
		<td class="label">
			<label for="dt_cast"><?php _d('Elenco'); ?></label>
		</td>
		<td class="field">
	...
	<tr id="dt_dir_box">
		<td class="label">
			<label for="dt_dir"><?php _d('Diretor'); ?></label>
		</td>
		<td class="field">
...
function custom_admin_js() {  global $post_type; if( $post_type == 'movies' || $post_type == 'pedido') { ?>
	...
	$('input[name=Checkbx]').click(function() {
		...
		$.getJSON(url + input + agregar + idioma + apikey, function(tmdbdata) {
			...
			$.each(tmdbdata, function(key, val) {
				...
				$('#message').remove();
				$('#postbox-container-2').prepend('<div id=\"message\" class=\"notice rebskt updated \"><p><?php if(dt_get_meta('ids')){ _d("Os dados foram atualizados, verifique por favor"); } else { _d("Os dados foram preenchidos, verifique por favor"); } ?></p></div>');
				$("#verificador").show();
				if (key == "title") {
				...
				if(key == "poster_path"){
					valupimg+= "https://image.tmdb.org/t/p/w600"+val+"";
