...
<table class="options-table-responsive dt-options-table">
	<tbody>
		...
		<tr id="episode_box">
			<td class="label">
				<label for="episode"><?php _e( 'Último Episódeo', 'mtms' ); ?></label>
			</td>
			<td class="field">
				<input class="regular-text" type="text" name="episode" id="episode" value="<?php echo dt_get_meta( 'episode' ); ?>">
			</td>
		</tr>
		<tr id="runtime_box">
			<td class="label">
...
<?php if (has_post_thumbnail()): else: echo '<input type="hidden" id="url_image_upload" name="url_image_upload" value="">'; endif; ?>
	...
	function series_save( $post_id ) {
		...
		if ( isset( $_POST['episode'] ) ) update_post_meta( $post_id, 'episode', esc_attr( $_POST['episode'] ) );
    ...
    jQuery.getJSON(url + input + agregar + idioma + apikey, function(tmdbdata) {
    	...
    	jQuery.each(tmdbdata, function(key, val) {
    		...
    		<?php if(dt_get_meta( 'ids' )){ ?>
    			...
    			jQuery('#postbox-container-2').prepend('<div id=\"message\" class=\"notice rebskt updated \"><p><?php _e("Os dados foram atualizados, verifique por favor","mtms"); ?></p></div>');
    			<?php } else { ?>
    				jQuery('#message').remove();
    				jQuery('#postbox-container-2').prepend('<div id=\"message\" class=\"notice rebskt updated \"><p><?php _e("Os dados foram preenchidos, verifique por favor","mtms"); ?></p></div>');
    				<?php } ?>
    				if (key == "name") {
    				...
    				if (key == "poster_path") {
    					jQuery.each(tmdbdata.seasons, function(i, item) {
    						if(i>99) return false;
    						var e = 0;
    						while (e < i){
    							valImg = item.poster_path;
    							e++;
    						}
    					});
    				}
    				...
    				if(key == "seasons"){
    					jQuery.each(tmdbdata.seasons, function(i, item) {
    						...
    						var e = 0;
    						while (e < i){
    							valupimg = "https://image.tmdb.org/t/p/w396"+item.poster_path+"";
    							e++;
    						}
    					});
    				}
    				<?php $api = get_option('dt_api_genres'); if ($api == "true") { ?>
    					...
    					if (key == "seasons") {
    						...
    						jQuery.each(tmdbdata.seasons, function(i, item) {
    							...
    							var e = 0;
    							while (e < i){
    								imgt = item.episode_count;
    								e++;
    							}
    						});
    						jQuery('#episode').val(imgt);
    					}
