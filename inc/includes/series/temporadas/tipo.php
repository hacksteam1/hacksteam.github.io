...
function seasons() {
	...
	$labels = array(
		'name'                => _x('Temporadas', 'Nome do tipo de postagem','mtms'),
		'singular_name'       => _x('Temporadas', 'Nome Tipo Singular','mtms'),
		'menu_name'           => __d('Temporadas'),
		'name_admin_bar'      => __d('Temporadas'),
	);
	$rewrite = array(
	...
	$args = array(
		'label'               => __d('Temporadas'),
		'description'         => __d('Gerir Temporadas'),
		'labels'              => $labels,
...
function seasons_table_head( $defaults ) {
	...
	$defaults['generate']    = __d('Gerar');
	endif;
	$defaults['serie']    = __d('Séries');
    $defaults['idtmdb']    = __d('ID TMDb');
	$defaults['dtviews'] = __d('Visualizações');
    return $defaults;
}
...
function seasons_table_content( $column_name, $post_id ) {
    ...
    echo '<a href="'. $addlink .'" class="dtload button button-primary">'. __d('Gerar episódios') .'</a>';
		}
		endif;
    }
