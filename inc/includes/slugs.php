...
class dt_slugs {
	...
	*/
	public function settingsInit() {
		...
		$this->addField('dt_author_slug', array( $this, 'author_slug'), __d('User name') );
		$this->addField('dt_filmes_slug', array( $this, 'filmes_slug'), __d('Movies') );
		$this->addField('dt_pedido_slug', array( $this, 'pedido_slug'), __d('Requests') );
		$this->addField('dt_series_slug', array( $this, 'series_slug'), __d('Series') );
		...
		$this->addField('dt_cast_slug', array( $this, 'cast_slug'), __d('Stars') );
		$this->addField('dt_creator_slug', array( $this, 'creator_slug'), __d('Author') );
	...
	*/
	public function slug_title() {
		echo '<h3>'. __d('Vizer: Permalink Configurações') .'</h3>';
	}
	...
	public function filmes_slug() {
		echo $this->input('dt_filmes_slug', 'filmes', '/titanic/');
	}
	public function pedido_slug() {
		echo $this->input('dt_pedido_slug', 'pedido', '/titanics/');
	}

	public function series_slug() {
		echo $this->input('dt_series_slug', 'series', '/the-walking-dead/');
	}
	...
	*/
	public function settingsSave() {
		...
		$this->saveField('dt_filmes_slug');
		$this->saveField('dt_pedido_slug');
		$this->saveField('dt_series_slug');
...
$dt_filmes_slug		= get_option('dt_filmes_slug');
$dt_pedido_slug		= get_option('dt_pedido_slug');
$dt_series_slug	= get_option('dt_series_slug');
...
if(empty($dt_filmes_slug)){
	update_option('dt_filmes_slug', 'filmes');
}

if(empty($dt_series_slug)){
	update_option('dt_series_slug', 'series');
}
...
if(empty($dt_pedido_slug)){
	update_option('dt_pedido_slug', 'pedido');
}
