...
define('MENU_TITLE', 'Vizer Opções'); 
define('MENU_SLUG', 'vizer');
if (!class_exists('acera_theme_options')) {
	class acera_theme_options {
		   ...
		   jQuery('#acera-settings').submit(function() { 
			  jQuery(this).ajaxSubmit({
				 success: function(){
					...
					jQuery('.saveMessage').append("<div id=\"message\" class=\"notice rebskt updated \"><p><?php echo htmlentities(__d('Configurações salvas.'),ENT_QUOTES); ?></p></div>").show();
				 }, 
				 timeout: 5000
			  }); 
		...
		<p><?php _d('Licença inválida, é possível que algumas das opções não funcionem corretamente') ?> <a href="<?php echo admin_url(); ?>themes.php?page=<?php echo DT_THEME_SLUG; ?>-license"><?php _d('Aqui'); ?></a></p>
		</div>
		<?php endif ?>
		...
		<div id="acera-meta-info">
		<h2><a href="<?php echo DT_SUPPORT_FORUMS; ?>" target="_blank"><?php _d('Fórum de suporte'); ?></a></h2>
		<h2><a href="<?php echo DT_CHANGELOG; ?>" target="_blank"><?php _d('Registro de alterações'); ?></a></h2>
		</div>
		</div>
		<div id="acera-content">
		<div class="social_single_left">
			<ul class="lsocials">
				<li><a class="facebook" href="https://www.fb.com/lucaskfelipelipe" target="_blank"><i class="dashicons dashicons-facebook-alt"></i></a></li>
				<li><a class="twitter" href="https://twitter.com/lucasfelipelipe" target="_blank"><i class="dashicons dashicons-twitter"></i></a></li>
			</ul>
		</div>
		...
		<input name="publish" class="dt_button button button-primary button-large" type="submit" value="Salvar alterações" id="publish" />
		</div>
		<?php
		...
		<input name="save" class="dt_button button button-primary button-large" type="submit" value="<?php _d('Salvar alterações'); ?>" />
		</div>
		</div> 
		...
		<span type="button" class="acera_remove acera-button" id="remove_<?php echo $value['id']; ?>"><?php _d('Remover'); ?></span>
		<?php endif; ?>
